...
new_probe = r"""PROBE_SCRIPT = r'''#!/usr/bin/env python3
import asyncio
import json
import logging
import time
...
async def main():
    ...
    await rmc.on_fill_update(signal_id, "YES", ask, size, "CONFIRMED")
...
if __name__ == "__main__":
    asyncio.run(main())
'''"""
text = re.sub(r"PROBE_SCRIPT = r'''[\s\S]*?'''", new_probe.strip(), text, count=1)
p.write_text(text, encoding="utf-8")
print("probe replaced")
